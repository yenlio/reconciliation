...
import { ReconciliationACVEntity } from '../../../core/entity';
...
export class ACVComponent implements OnInit {
  ...
  datasTable: ReconciliationACVEntity[] = [];
  dataStatus: string = '';
  showForm = false;
  ...
  cacheItem = {};
  cacheData: any = [];
  saveCache(item: ReconciliationACVEntity) {
    this.cacheItem = item;
  ...
  search() {
    this.reconciliationRepositoryService
      .getReconciliationPartnerACV(this.filter)
      .subscribe((datas: ResponseGetReconciliationACVDto) => {
  ...
  acceptPartnerResult(item: ReconciliationACVEntity) {
    if (!this.isACV) {
      item.rcEpayAmount = item.rcAcvAmount;
      item.rcEpayStatus = item.rcACVStatus;
      this.cacheData.push(item);
    } else {
      item.rcAcvAmount = item.rcEpayAmount;
      item.rcACVStatus = item.rcEpayStatus;
      this.cacheData.push(item);
    }
  }
  ...
  confirm() {
    ...
    if (item.length > 0) {
      this.reconciliationRepositoryService
        .updateReconciliationsPartnerACV(this.datasTable)
        .subscribe((datas: ReconciliationACVEntity[]) => {
  ...
  checkRole() {
    ...
    this.isACV = roles.every((role: any) => role == 'ROLE_ACV');
  }
}
